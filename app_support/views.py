...
from helpers.emails import send_email_verification_link, send_plan_change_confirmation, \
	send_plan_assigned_confirmation, send_plan_renew_confirmation
from helpers.db import register_or_verify_subscriber, get_user_details, get_all_users, \
	get_user_by_id, get_user_by_email, update_user_status, get_plan_by_id, \
	get_all_plan_change_requests, get_all_user_plan_history, \
	...
	def post(self, request, format=None):
		...
		if 'user_id' in request.POST.keys():
			...
			if len(user_set) > 0:
				...
				email_verification_url = '{}/api/account/confirm_email?verification_code={}&format=json'.format(get_host_origin(request),
																							token)
