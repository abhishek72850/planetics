...
from app_perf.models import Subscribers, SubscriberPlanModel, PlanChangeRequestModel, UpcomingPlanModel, UserPlanHistoryModel
...
def update_topic_quota_counter(user):
    success = Subscribers.objects.filter(id=user.id).update(
        topic_quota_counter=user.topic_quota_counter + 1
    )
...
def is_topic_quota_exhausted(user):
    ...
    count = user.topic_quota_counter
